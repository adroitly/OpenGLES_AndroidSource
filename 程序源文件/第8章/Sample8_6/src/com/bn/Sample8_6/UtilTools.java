...
public class UtilTools {
	...
	float r = 0;// 足球碳的半径
	int count = 0;
	   public ResultData initVertexData(float scale, float aHalf, int n) {//大小，黄金长方形的长的一半，分段数
		...
		ArrayList<Integer> alFaceIndex20 = new ArrayList<Integer>();// 正20面体组织成面的顶点的索引值列表
		// 正20面体顶点
		...
		// 计算卷绕顶点
		float[] vertices20 = VectorUtil.cullVertex(alVertix20, alFaceIndex20);// 只计算顶点,构成二十面体的三角形顶点坐标
		...
		for (int k = 0; k < vertices20.length; k += 9){// 对正20面体每个大三角形循环
			...
			float[] v3 = new float[] { vertices20[k + 6], vertices20[k + 7],
					vertices20[k + 8] };
			// 算出每个三角形每条边上的两个顶点所在大圆圆弧上的三等分点坐标
			for (int i = 1; i < n; i++) {
				float[] vi1 = VectorUtil.devideBall(r, v1, v2, n, i);//大圆圆弧上的三等分点坐标
				vi1 = VectorUtil.normalizeVector(vi1);// 规格化vi1
				float[] vi2 = VectorUtil.devideBall(r, v1, v3, n, i);//大圆圆弧上的三等分点坐标
				vi2 = VectorUtil.normalizeVector(vi2);// 规格化vi2
				float[] vi3 = VectorUtil.devideBall(r, v2, v3, n, i);//大圆圆弧上的三等分点坐标
			...
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6));	 // 1号顶点的x,y,z坐标
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6 + 1)); // 2号顶点的x,y,z坐标
			
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6 + 1)); // 2号顶点的x,y,z坐标
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6 + 4)); // 5号顶点的x,y,z坐标
			
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6 + 4)); // 5号顶点的x,y,z坐标
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6 + 5)); // 6号顶点的x,y,z坐标
			
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6 + 5)); // 6号顶点的x,y,z坐标
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6 + 2)); // 3号顶点的x,y,z坐标
			
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6 + 2)); // 3号顶点的x,y,z坐标
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6 + 3)); // 4号顶点的x,y,z坐标
			
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6 + 3)); // 4号顶点的x,y,z坐标
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6)); // 1号顶点的x,y,z坐标
			count+=1;
		}
		// 创建化学键的边的端点坐标数组，ChemicalBondPoints中每个一维数组存放兩个顶点信息
		...
		for (int i = 0; i < Rda.ChemicalBondPoints.length; i++) {
			// 把AlChemicalBondPoints中的第2*i个点的坐标转存到Rda.ChemicalBondPoints
			for (int j = 0; j < 3; j++) {
				Rda.ChemicalBondPoints[i][j] =r * AlChemicalBondPoints.get(2 * i)[j];
			}
			// 把AlChemicalBondPoints中的第2*i+1个点的坐标转存到Rda.ChemicalBondPoints
			for (int k= 3; k <6; k++) {
				Rda.ChemicalBondPoints[i][k] = r *AlChemicalBondPoints.get(2 * i + 1)[k - 3];
			}
		}
		// 创建碳原子的坐标数组Rda.CAtomicPosition
